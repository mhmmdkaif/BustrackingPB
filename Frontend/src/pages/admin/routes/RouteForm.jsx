...
export default function RouteForm({ selectedRoute }) {
  ...
  useEffect(() => {
    if (selectedRoute && user?.token) {
      fetchRouteStops(selectedRoute.id, user.token);
    } else {
      setRouteStops([]);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedRoute?.id, user?.token]);

  const fetchRouteStops = useCallback(async (routeId, token) => {
    try {
      setLoading(true);
      setError("");
      ...
      setError(
        err?.response?.data?.message || "Failed to load route stops."
      );
    } finally {
      setLoading(false);
    }
