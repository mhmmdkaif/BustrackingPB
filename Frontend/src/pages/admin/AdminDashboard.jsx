...
export default function AdminDashboard() {
  ...
  // ✅ FIX: wait for token before fetching stats
  useEffect(() => {
    if (user?.token) {
      fetchStats();
    }
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <SnapshotRow
              ...
              value={stats.routes}
              description="Defined paths available for trip planning."
            />
            <SnapshotRow
              label="Registered stops"
              value={stats.stops}
              description="Bus stops with coordinates for tracking."
            />
            <SnapshotRow
              label="Fleet size"
              value={stats.buses}
              description="Total buses available in the system."
            />
            <SnapshotRow
              label="Active drivers"
              value={stats.drivers}
              description="Drivers linked to user accounts."
            />
          </div>
        </div>
